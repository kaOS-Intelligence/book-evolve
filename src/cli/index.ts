...
function projectPython(projectDir: string): string {
 ...
 */
function applyProjectEnv(projectDir: string): void {
  const applied = loadEnvAi(projectDir);
  if (applied === null) return;
  if (applied.length > 0) {
    console.log(
      chalk.dim(`  Loaded .env.ai (${applied.length} vars) from ${projectDir}`),
    );
  }
}

/** Light config.yaml reader for book metadata (no YAML dependency needed). */
function readBookMeta(projectDir: string): {
  title: string;
  author: string;
  goal: string;
} {
  ...
  if (!existsSync(configPath)) return meta;
...
program
  .name('book-evolve')
  .description(
    'Developmental editor — evolve dictated transcripts into polished book prose',
  )
  .version(PACKAGE_VERSION)
  .option(
    '--web',
    'Launch the web-based onboarding wizard at http://localhost:3010',
  )
  .option('--skip-onboard', 'Skip interactive onboarding, use defaults')
  .option(
    '--output <dir>',
    'Output directory for the book project',
    './book-project',
  )
  .action(async (options) => {
    ...
    console.log(
      chalk.dim('  Developmental Editor — 3-Model Evolutionary Council'),
    );
    console.log('');
    ...
    if (existing.exists) {
      ...
      if (options.skipOnboard) {
        ...
        console.log(
          chalk.yellow('  Skipping onboarding — using default configuration.'),
        );
      } else {
      ...
      console.log(
        chalk.dim(
          `  cd ${options.output} && npx @kaos-intelligence/book-evolve doctor`,
        ),
      );
      console.log('');
    }
...
program
  .command('setup')
  .description(
    'Create the Python environment and install pipeline dependencies',
  )
  ...
  .action((options) => {
    ...
    if (!existsSync(setupScript)) {
      console.error(
        chalk.red('setup.sh not found — is the pipeline extracted?'),
      );
      console.error(
        chalk.dim(
          'Run: npx @kaos-intelligence/book-evolve scaffold --dir . --force',
        ),
      );
      process.exitCode = 1;
      return;
    ...
    if (result.status === 0) {
      ...
      console.log(
        chalk.dim(
          '  1. cp .env.ai.example .env.ai and set LITELLM_BASE_URL + your models',
        ),
      );
      console.log(chalk.dim('  2. npx @kaos-intelligence/book-evolve doctor'));
      console.log(
        chalk.dim('  3. npx @kaos-intelligence/book-evolve evolve --chapter 1'),
      );
      console.log('');
    }
...
program
  .command('doctor')
  .description(
    'Verify the project, Python environment, model endpoint, and content are ready',
  )
  .option('--project <dir>', 'Project directory', '.')
  .action(async (options) => {
    const projectDir = resolve(options.project);
...
program
  .command('smoke')
  .description(
    'Live smoke test — one tiny completion per council/judge model plus an embedding round-trip',
  )
...
program
  .command('evolve')
  .description(
    'Run the evolution pipeline (seed → provision → evolve → promote)',
  )
  .option('--project <dir>', 'Project directory', '.')
  .option('--chapter <n>', 'Chapter number to evolve', '1')
  .option(
    '--end-chapter <n>',
    'Evolve through this chapter (defaults to --chapter)',
  )
  ...
  .action(async (options) => {
    ...
    if (!process.env.ASI_EVOLVE_HASH_EMBEDDINGS) {
      ...
      if (!ollamaUp) {
        console.error(
          chalk.red(
            `Embeddings unavailable — Ollama unreachable at ${ollamaUrl}.`,
          ),
        );
        console.error(
          chalk.dim(
            'The evolution engine needs embeddings for retrieval. Either:\n' +
    ...
    if (python === 'python3') {
      console.log(
        chalk.yellow(
          'No .venv found — run `book-evolve setup` first for a reliable environment.',
        ),
      );
    }
    ...
    console.log(
      chalk.cyan(
        `Evolving chapters ${startChapter}..${endChapter} of "${meta.title}"...`,
      ),
    );
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        console.log(
          chalk.green(
            `Chapters ${startChapter}..${endChapter} evolved successfully.`,
          ),
        );
        console.log(
          chalk.dim(`Output: ${join(projectDir, 'output', 'chapters')}`),
        );
      } else {
        console.error(chalk.red(`Evolution failed with exit code ${code}.`));
        console.error(
          chalk.dim('Run `book-evolve doctor` to check your environment.'),
        );
