...
export async function launchProvider(
  ...
  try {
    ...
    console.log('');
    console.log(
      chalk.yellow('  Fallback: Open the project in your AI of choice and'),
    );
    console.log(
      chalk.yellow(
        `  paste the prompt from: ${join(projectPath, 'book-evolve.md')}`,
      ),
    );
    console.log('');
  }
...
async function launchCursor(
  ...
  console.log(
    chalk.dim('  @book-evolve in chat to activate the developmental editor.'),
  );
  console.log('');
...
async function launchClaude(
  ...
  console.log(
    chalk.dim(`  Project config: ${join(projectPath, 'claude-project.json')}`),
  );
  console.log('');
  console.log(chalk.dim('  In Claude Desktop, open the project folder and'));
  console.log(
    chalk.dim(
      `  paste the contents of: ${join(projectPath, 'book-evolve.md')}`,
    ),
  );
}
...
async function launchAPI(
  ...
  console.log(
    chalk.dim(
      `  Environment template: ${join(projectPath, '.env.ai.example')}`,
    ),
  );
  console.log('');
  console.log(
...
function printNextSteps(config: ProjectConfig, projectPath: string): void {
  console.log('');
  console.log(chalk.bold.green('  NEXT STEPS'));
  console.log(chalk.dim('  ──────────'));
  console.log('');
  console.log(
    `${chalk.white('  1. ')}Place your dictation .txt files in:${chalk.dim(` ${config.dictationDir}`)}`,
  );
  console.log(
    `${chalk.white('  2. ')}Place your reference book .txt files in:${chalk.dim(` ${config.referenceDir}`)}`,
  );
  console.log(
    `${chalk.white('  3. ')}Seed the cognition store:${chalk.dim(' npx @kaos-intelligence/book-evolve seed --project .')}`,
  );
  console.log(
    `${chalk.white('  4. ')}Start the conversation by saying:${chalk.bold(' "Begin Chapter One."')}`,
  );
  console.log('');
  console.log(
    chalk.dim(`  Full prompt: ${join(projectPath, 'book-evolve.md')}`),
  );
  console.log('');
}
