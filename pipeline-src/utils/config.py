...
def _resolve_env_vars(obj: Any) -> Any:
    ...
    `${VAR}` resolves to the environment value, or "" when unset.
    `${VAR:-default}` resolves to the environment value, or `default` when
    unset or empty (shell parameter-expansion semantics).
    """
    if isinstance(obj, dict):
        ...
        return value if value else (default if sep else "")
    return obj
