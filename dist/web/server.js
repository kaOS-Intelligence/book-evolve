...
const PORT = 3010;
export async function startWebUI(outputDir) {
            ...
            req.on('end', async () => {
                try {
                    ...
                    const config = {
                        ...
                        bookGoal: (raw.bookGoal || 'Evolve raw dictation into polished prose.').trim(),
                        dictationDir: raw.dictationDir || join(outputDir, 'dictation'),
                        referenceDir: raw.referenceDir || join(outputDir, 'author-style'),
                        provider: (raw.provider || 'terminal'),
                        outputDir: raw.outputDir || outputDir,
                        targetScore: 0.93,
                        patience: 8,
                        maxSteps: 10,
                    };
                    const scaffoldPath = await scaffoldProject(config, raw.force === true);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ ok: true, path: scaffoldPath }));
                    console.log('');
                    console.log(chalk.green(`  Project at: ${scaffoldPath}`));
                    console.log(chalk.dim('  Wizard still running — scaffold more projects, or press "Done" (or Ctrl-C) to exit.'));
                }
                catch (err) {
    ...
    return new Promise((resolve) => {
        server.listen(PORT, () => {
            console.log('');
            console.log(chalk.bold.green(`  Book Evolution Onboarding running at http://localhost:${PORT}`));
            console.log(chalk.dim('  Fill in the form to scaffold your book project.'));
            console.log('');
            // Try to open browser
...
function getOnboardingHTML(outputDir) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
