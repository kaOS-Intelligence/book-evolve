...
const litellmUrl = () => process.env.LITELLM_BASE_URL || 'http://127.0.0.1:4000/v1';
...
function checkPipeline(projectDir) {
    ...
    const enginePresent = existsSync(runner) &&
...
function checkVenv(projectDir) {
    ...
    const result = spawnSync(venvPython, ['-c', 'import openai, yaml, jinja2, numpy, requests'], { encoding: 'utf-8', timeout: 30_000 });
    return {
        name: 'Python environment',
        ok: result.status === 0,
...
async function checkEndpoint(name, url, fix) {
...
async function checkOllamaEmbedModel() {
    ...
    try {
        ...
        const hasModel = (data.models || []).some((m) => m.name === embedModel() || m.name.startsWith(`${shortName}:`));
        return {
            name,
            ok: hasModel,
            detail: hasModel
...
function checkContent(projectDir) {
...
export async function runSmoke() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const reply = (data.choices?.[0]?.message?.content || '')
                    .trim()
                    .slice(0, 40);
                console.log(`  ${chalk.green('✓')} ${chalk.bold(model)} — "${reply}" (${elapsed}s)`);
            }
            else {
                const body = (await res.text()).slice(0, 120);
                console.log(`  ${chalk.red('✗')} ${chalk.bold(model)} — HTTP ${res.status}: ${body}`);
                allOk = false;
            }
        }
        catch (err) {
            console.log(`  ${chalk.red('✗')} ${chalk.bold(model)} — ${describeError(err)}\n      ${chalk.dim('Is LITELLM_BASE_URL pointing at a running OpenAI-compatible endpoint with this model routed?')}`);
            allOk = false;
        }
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            const ok = dim > 0;
            console.log(`  ${ok ? chalk.green('✓') : chalk.red('✗')} ${chalk.bold(embedLabel)} — ${dim}-dim vector (${elapsed}s)`);
            if (!ok)
                allOk = false;
        }
        else {
            console.log(`  ${chalk.red('✗')} ${chalk.bold(embedLabel)} — HTTP ${res.status}`);
            allOk = false;
        }
    }
    catch (err) {
        console.log(`  ${chalk.red('✗')} ${chalk.bold(embedLabel)} — ${describeError(err)}`);
        allOk = false;
    }
    console.log('');
    if (allOk) {
        console.log(chalk.green.bold('  Smoke test passed — the full model path is live.'));
    }
    else {
...
export async function runDoctor(projectDir) {
    ...
    console.log(chalk.dim(`  Judge: ${models.judge}   Fast: ${models.fast}   Embeddings: ${models.embed}`));
    console.log('');
    const results = [
        checkPython(),
        checkPipeline(projectDir),
        checkVenv(projectDir),
        await checkEndpoint('Model endpoint', `${litellmUrl()}/models`, 'Point LITELLM_BASE_URL at any OpenAI-compatible endpoint with your council models routed (LiteLLM proxy, vLLM, OpenRouter, ...).'),
    ...
    if (allRequired) {
        ...
        console.log(chalk.dim('  Run: npx @kaos-intelligence/book-evolve evolve --project . --chapter 1'));
    }
    else {
