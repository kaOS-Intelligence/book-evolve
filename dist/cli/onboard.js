...
export async function onboard(defaultOutput) {
    console.log(chalk.bold("  Let's set up your book project."));
    console.log(chalk.dim("  I'll ask a few questions, then scaffold everything for you."));
    console.log('');
    const { bookTitle } = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().length > 0 ? true : 'Please enter a title.',
        },
    ]);
    const { bookGoal } = await inquirer.prompt([
        {
            type: 'input',
            name: 'bookGoal',
            message: 'One-sentence goal or description:',
            default: 'Evolve raw dictated transcripts into polished, publishable prose.',
        },
    ]);
    ...
    if (dictationFiles.length > 0) {
        console.log(chalk.dim(`  Found ${dictationFiles.length} dictation file(s): ${dictationFiles.join(', ')}`));
    }
    else {
        console.log(chalk.yellow(`  No dictation files found in ${dictationDir}. Drop your .txt files there.`));
    }
    if (referenceFiles.length > 0) {
        console.log(chalk.dim(`  Found ${referenceFiles.length} reference file(s): ${referenceFiles.join(', ')}`));
    }
    else {
        console.log(chalk.yellow(`  No reference files found in ${referenceDir}. Drop your .txt files there.`));
    }
    return {
